...
public class EventRunsProcessor implements Processor<String, RunDetails> {
	...
	public void init(ProcessorContext context) {
		...
		while(iter1.hasNext()) {
			KeyValue<String, EventResultDetails> keyValue=iter1.next();
	...
	public void process(String key, RunDetails value) {
		...
		resultMapForEvent.entrySet().stream()
		.sorted((e1,e2)->Long.valueOf(e2.getValue().getRunTotalTime())-Long.valueOf(e1.getValue().getRunTotalTime())>0?1:-1)
		.map((e)->{
