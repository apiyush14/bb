...
public interface EventDetailsRepository extends JpaRepository<EventDetails, Long> {
	...
	@Query(nativeQuery=true, value="SELECT * from Event_Details e WHERE e.IS_EVENT_APPROVED='Y' and e.EVENT_END_DATE<=DATE_SUB( CURRENT_TIMESTAMP, INTERVAL 15 MINUTE ) and e.EVENT_START_DATE >= DATE(NOW()) - INTERVAL 7 DAY and e.IS_CREDIT_CALCULATED<>'Y' ")
