...
public class UpdateCreditsFork extends RecursiveAction {
    ...
    protected void compute() {
    	...
    	List<UpdateCreditsFork> subtasks =
                new ArrayList<UpdateCreditsFork>();
	...
	private Collection<? extends UpdateCreditsFork> createSubtasks() {
		...
		for(EventResultDetails erd : this.eventResultDetailsList) {
			...
			UpdateCreditsFork subtask = new UpdateCreditsFork(erd, eventResultDetailsList, runSummaryList, runDetailsList);
	...
	private void updateCredits(List<RunDetails> runDetailsList2, List<RunSummary> runSummaryList2, List<EventResultDetails> eventResultDetailsList, EventResultDetails eventResultDetailsObj) {
		
              System.out.println(Thread.currentThread().getName() + "START - Updating Credits for User ID - " 
           +eventResultDetailsObj.getUserId()+"for event id - "+eventResultDetailsObj.getEventId()+" at "+ new Date());
              Long userRank = eventResultDetailsObj.getUserRank();
         double multiplier = 0.6;
		Double  runCreditsForUser=   ((1 - multiplier) / (1 - Math.pow(multiplier , eventResultDetailsList.size())) * Math.pow(multiplier, (userRank- 1))) * totalCreditsForEvent;
		...
		eventResultDetailsObj.setRunCredits(runCreditsForUser);
 		...
 		Optional<RunDetails> runDetails = runDetailsList2.stream().filter(runDet -> (runDet.getUserId().equals(eventResultDetailsObj.getUserId()) && 
 				runDet.getRunId().equals(eventResultDetailsObj.getRunId()) )).findFirst();
 		if(runDetails.isPresent()) {
 			runDetails.get().setRunCredits(runCreditsForUser);
 		}
        
 		
		Optional<RunSummary> existingRunSummaryOptional = runSummaryList2.stream().filter(runSum -> (runSum.getUserId().equals(eventResultDetailsObj.getUserId()))).findFirst();
