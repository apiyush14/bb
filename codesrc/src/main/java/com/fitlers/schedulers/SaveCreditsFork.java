...
public class SaveCreditsFork extends RecursiveAction {
    ...
    public SaveCreditsFork(boolean isNew, List<EventResultDetails> eventResultDetailsList,
			List<RunSummary> runSummaryList, List<RunDetails> runDetailsList, EventResultDetailsRepository eventResultDetailsRepository, 
    ...
    public SaveCreditsFork(EventResultDetails eventResultDetails, RunSummary runSummary, RunDetails runDetails,EventResultDetailsRepository eventResultDetailsRepository, 
				RunDetailsRepository runDetailsRepo, RunSummaryRepository runSummaryRepository) {
...
this.runSummaryRepository=runSummaryRepository;
    ...
    protected void compute() {
    	...
    	if(this.isNew) {
    	List<SaveCreditsFork> subtasks =
                new ArrayList<SaveCreditsFork>();
	...
	private Collection<? extends SaveCreditsFork> createSubtasks() {
		...
		for(EventResultDetails erd : this.eventResultDetailsList) {
			...
			Optional<RunSummary> runSummary = runSummaryList.stream().filter(runSummaryList -> runSummaryList.getUserId().equals(erd.getUserId())).findFirst();
			Optional<RunDetails> runDetails = runDetailsList.stream().filter(runDetailsList -> runDetailsList.getUserId().equals(erd.getUserId())).findFirst();
			SaveCreditsFork subtask = new SaveCreditsFork(erd, runSummary.get(), runDetails.get(), eventResultDetailsRepository, runDetailsRepo, runSummaryRepository);
